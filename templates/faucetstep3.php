            ...
            if(checkDirtyIp($realIpAddressUser, $iphubApiKey) == true AND $VPNShield == "yes"){
                ...
                if($IpCheck >= 1){
                    echo alert("danger", "Someone else claimed in your network already.");
                        ...
                        $mysqli->query("UPDATE users Set balance = balance + $payOutBTC, last_claim = '$timestamp' WHERE id = '{$user['id']}'");
                        $mysqli->query("UPDATE settings SET value = value + 1 WHERE name = 'total_claims'");
                        echo "<a href='index.php?page=dashboard' class='btn btn-dark btn-lg custom-button'>You've claimed successfully ".$payOut." {$faucetCurrencies[$websiteCurrency][1]}.</a>";
